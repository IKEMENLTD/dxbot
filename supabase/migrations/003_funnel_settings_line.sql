...
CREATE OR REPLACE FUNCTION update_recommendation(
  p_user_id UUID, p_exit_type exit_type, p_score NUMERIC(5,2),
  p_reason TEXT DEFAULT '', p_confidence NUMERIC(5,2) DEFAULT 0, p_reasons JSONB DEFAULT '[]'
)
RETURNS VOID LANGUAGE plpgsql AS $$
BEGIN
  INSERT INTO recommend_scores (user_id, exit_type, score, reason, confidence, reasons, calculated_at)
  VALUES (p_user_id, p_exit_type, p_score, p_reason, p_confidence, p_reasons, NOW())
...
--   SELECT u.id, 'note_added', '週次リマインド自動送信',
